          ...
          @foreach($posts as $post)
            ...
            <td>
                ...
                <form action="/dashboard/posts/{{ $post->slug }}" method="POST" class="d-inline">
                    @method('delete')
                    @csrf                    
                <button class="badge bg-danger border-0" onclick="return confirm('Are you want to delete your post with a title : {{ $post->title }} ?')" type="submit"><span data-feather="x-circle" class="align-text-bottom"></span></button>
                </form>
            </td>
            </tr>
        @endforeach
